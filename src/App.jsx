...
function LocalInput({ value, onChange, ...props }) {
  ...
  return <input {...props} value={val}
    onFocus={()=>{focused.current=true;}}
    onChange={e=>setVal(e.target.value)}
    onBlur={()=>{focused.current=false;onChange(val);}} />;
}
...
function ScenarioEditor({ scenarios, setScenarios, projects, scenarioId, updateScenario, onBack, generateMapSVGString }) {
  ...
  // ── SOLUTION DÉFINITIVE : état 100% local, sauvegarde uniquement au retour ──
  const initialScenario = scenarios.find(s=>s.id===scenarioId);
  const [local, setLocal] = useState(() => initialScenario ? JSON.parse(JSON.stringify(initialScenario)) : null);
  
  // Ref pour toujours avoir la dernière version disponible pour le save
  const localRef = useRef(local);
  useEffect(() => { localRef.current = local; }, [local]);
  
  // Sauvegarde vers le parent
  function saveToParent() {
    if (localRef.current) updateScenario(scenarioId, localRef.current);
  }
  
  // Sauvegarde automatique toutes les 5 secondes (sécurité)
  useEffect(() => {
  ...
  function upd(patch) { setLocal(prev => ({ ...prev, ...patch })); }
  
  function handleBack() { saveToParent(); onBack(); }
  ...
  return (
    <div style={{...cssVars,background:"var(--bg-dark)",height:"100vh",fontFamily:"'Segoe UI',system-ui,sans-serif",color:"var(--text-primary)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"center",padding:"6px 12px",background:"var(--bg-panel)",borderBottom:"1px solid var(--border)",gap:8,flexWrap:"wrap"}}>
        <button onClick={handleBack} style={{background:"none",border:"none",color:"var(--accent)",fontSize:18,cursor:"pointer"}}>←</button>
